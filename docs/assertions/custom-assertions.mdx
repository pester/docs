...
- *Remaining bound parameters*, typically matching the parmaeters your accept in your assertion, like `ExpectedValue`, `Because`, `MyCustomParameter` etc.
...
Advanced functions, typically enabled by using `[CmdletBinding()]` or `[Parameter()]`, disallows arguments by default. In this scnario, make sure to accept all mandatory parameters in your param-block to avoid errors even if you don't use them. They are `ActualValue`, `Negate` and `CallerSessionState`.
::: 
