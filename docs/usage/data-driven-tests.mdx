...
There are two main things to take into account when Data driven tests in Pester v5: The execution is not top down. And variables from `Discovery` are not available in `Run`, unless you explicitly attach them to a `Describe`, `Context` or `It`.
