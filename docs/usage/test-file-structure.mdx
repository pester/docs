...
```
See [Generating tests](generating-tests).
...
There is a special block called `BeforeDiscovery` that shows intent when you need to put code directly in the script, or directly in `Describe` or `Context` block. This is useful when generating tests.
