...
:::tip See which tags are on which test
Set [`Output.ShowTags`](./output#showing-tags) to print the tags next to each `Describe`, `Context` and `It` in the console output, so you can see what your filter is actually matching.
:::
