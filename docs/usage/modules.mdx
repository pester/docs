...
However injecting mocks into or executing code inside the module are not available for **Binary** modules due to how they are implemented in PowerShell. As a result, you may see an error message when trying to use `Mock -ModuleName` or `InModuleScope`:
