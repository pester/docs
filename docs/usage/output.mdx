...
This option controls the general level of console output. Pester supports the following levels:
...
When an error is shown due to a failed setup or test, a stacktrace is included to help you identify where the problem occurred. Users might not care much about this while developers need it to identify and fix any issues.
...
`Output.ShowTags` appends the tags of each `Describe`, `Context` and `It` to its output line. It makes it easy to see what a `-TagFilter` or `-ExcludeTagFilter` is actually matching:

```powershell
$conf = New-PesterConfiguration
$conf.Run.Path = '.'
$conf.Output.Verbosity = 'Detailed'
$conf.Output.ShowTags = $true
Invoke-Pester -Configuration $conf
```

```
Describing Get-Planet [Tags: Slow, Unix]
  [+] returns all planets [Tags: Fast] 34ms
```

Blocks and tests without tags are printed as before. See [Tags](./tags) for how filtering works.
