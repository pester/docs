...
- **Auto (default)**: Automatically enables CI-specific format when Pester detects that it runs in supported CI-system. If not, Pester will use the normal Pester output like `None`.
- **AzureDevops**: Enables Azure Devops-specific format to log errors and show the error messages in red.
- **GithubActions**: Enables Github Actions-specific format to log errors and show the error message in red. Error message and stacktrace are grouped.
...
#### Github Actions
In Github Actions workflows, the following features are enabled:
