...
Describe "Get-Emoji" {
    It "Returns <expected> (<name>)" -TestCases @(
        ...
        Get-Emoji -Name cactus | Should -Be $expected
    }
}
...
- `It` function is about to be run, all its parameters need to be evaluated first, 
    - `-Name` is evaluated
    - `-TestCases` are evaluated
- `It` function runs. It saves the ScriptBlock provided to it, but does not execute it yet. One `It` is generated for each item in the `-TestCases` array.
...
Container: Get-Emoji.Tests.ps1 
    ...
    Blocks: 
        ...
        Tests: 
            ...
            ScriptBlock: { Get-Emoji -Name cactus | Should -Be $expected }
...
- Create new scope to insolate the test from other tests
...
In this case the `foreach` is evaluated during `Discovery` because we simply run the whole script file. But `BeforeAll` won't run until run `Run`, so the `$files` variable is not defined during `Discovery` and the tests are not generated. This can be solved by defining the code that is used to generate tests into `BeforeDiscovery` block. See [Generating tests].(generating-tests).
...
Both of the problems above are avoidable by using `-ForEach`, see [Generating tests].
