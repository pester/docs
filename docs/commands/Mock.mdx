...
implementation, for example to wrap or inspect its output.
Capture the
...
The parameters bound on the mocked
command are also available as a hashtable in the $PesterBoundParameters
variable, which is convenient for forwarding them to another command.
See
...
When the experimental Mock.Global
