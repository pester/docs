...
id: Should-BeNull
title: Should-BeNull
description: Help for Pester command 'Should-BeNull'. Asserts that the input is `$null`.
keywords:
  - PowerShell
  - Pester
  - Help
...
```powershell
Should-BeNull [[-Actual] <Object>] [-Because <string>]
```

## DESCRIPTION

This assertion passes only when the actual value is exactly `$null.
Empty strings, empty collections, and other falsy values do not count as null.
