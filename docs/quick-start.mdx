...
Pester can produce artifacts such as a Test Results file and can be used for generating
...
Now that we split the tests into groups we might want to share some common code among those tests. To do this we use a `BeforeAll` block that will run at the start of the block that contains it, or at the start of the file if not contained in any block. In our example we used it to define the tested function.
