...
```powershell
Invoke-Pester -Path ./Planetarium/Public/Export-PlanetReport.Tests.ps1
```
...
There is no `Should-Exist` in Pester v6, so file existence is asserted with `Test-Path` piped into `Should-BeTrue` or `Should-BeFalse`. The first test asserts the file is absent *before* the call as well as present after: without that, a leftover file from an earlier run could make the test pass on its own.
