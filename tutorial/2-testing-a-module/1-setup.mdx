...
The `.psm1` file is the module's root script, executed when your import the module. Ours does not define any functions itself; it finds the `.ps1` files, one per function, and dot-sources them.
...
Using `FunctionsToExport = '*'` is convenient while developing, but should be change to a array of public function names before publishing. This is required for best performance and auto-import.
...
The error above is expected for now, as there's no functions files in the `Public` folder yet.
