...
:::warning A mock without `-ModuleName` does not reach into the module
What you get depends on where else the command is visible.

For a command that also exists outside the module, like `Import-Csv`, nothing errors. The mock is simply never used, the real command runs, and the test passes or fails for reasons that have nothing to do with the mock you thought you installed.

For a command that only exists inside the module, like the private `Get-PlanetData`, `Mock` cannot find it at all:

```
CommandNotFoundException: Could not find Command Get-PlanetData
```

If a mock appears to have no effect, or `Mock` cannot find a command you know is there, `-ModuleName` is the first thing to check.
:::
